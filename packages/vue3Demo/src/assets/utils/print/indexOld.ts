...
G6.registerNode(
  "aggregated-node",
  {
    draw(cfg, group) {
      ...
      const keyShape = group.addShape("rect", {  //椭圆形 展示
        attrs: {
          ...style,
          x: -width / 2,
          y: -height / 2,
    ...
    setState: (name, value, item) => {  //获取锚点（相关边的连入点）
...
G6.registerNode(
  "real-node",
  {
    draw(cfg, group) {
      ...
      const keyShape = group.addShape("circle", {  // 圆形
        attrs: {
          ...style,
          x: 0,
          y: 0,
...
const processNodesEdges = (  //处理节点和边
  nodes,
  edges,
  width,
  height,
...
const getForceLayoutConfig = (graph, largeGraphMode, configSettings) => {
  ...
  const config = {
    ...
    nodeStrength: (d) => {  // 节点作用力，正数代表节点之间的斥力作用，负数代表节点之间的引力作用（注意与 'force' 相反）
      // 给离散点引力，让它们聚集
    ...
    nodeSize: (d) => {  // 节点大小（直径）。用于碰撞检测。
      if (!nodeSize && d.size) return d.size;
      return 50;
    },
    nodeSpacing: (d) => {  // preventOverlap 为 true 时生效, 防止重叠时节点边缘间距的最小值。可以是回调函数, 为不同节点设置不同的最小间距
      if (d.degree === 0) return nodeSpacing * 2;
      if (d.level) return nodeSpacing;
      return nodeSpacing;
    },
    onLayoutEnd: () => {  // 布局完成后的回调函数
      if (largeGraphMode) {
...
const getNeighborMixedGraph = (  //获取相邻节点数据
  centerNodeModel,
  step,
  originData,
